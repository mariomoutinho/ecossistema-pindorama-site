// Base da API. Prioriza override manual (localStorage) para desenvolvimento;
// em producao deriva o caminho /suinda/api a partir da URL atual (o app vive
// em /suinda/app/...), funcionando mesmo que /suinda esteja em subpasta.
function suindaResolveApiBaseUrl() {
  const override = localStorage.getItem("suinda_api_base_url");
  if (override) return override;
  const match = window.location.pathname.match(/^(.*\/suinda)\//);
  if (match) return window.location.origin + match[1] + "/api";
  return "http://127.0.0.1:8013";
}

const SUINDA_API_BASE_URL = suindaResolveApiBaseUrl();
